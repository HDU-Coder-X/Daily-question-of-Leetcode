...
```java
class Solution {
    private List<Integer> getRevisions(String version){
    ...
    public int compareVersion(String version1, String version2) {
        List<Integer> revisions1 = getRevisions(version1);
        List<Integer> revisions2 = getRevisions(version2);
        int n1 = revisions1.size();
        int n2 = revisions2.size();
        int length = Math.max(n1,n2);
        for(int i = 0;i < length;i++){
            int r1 = i < n1 ? revisions1.get(i) : 0;
            int r2 = i < n2 ? revisions2.get(i) : 0;
            if(r1 == r2) 
                continue;
            else if(r1 < r2)
                return -1;
